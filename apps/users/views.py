...
from lzkshop.settings import APIKEY
...
class UserViewset(CreateModelMixin,mixins.RetrieveModelMixin,mixins.UpdateModelMixin,viewsets.GenericViewSet):
    ...
    # 这里需要动态选择用那个序列化【动态序列化分配】
    # 1、UserRegSerializer（用户注册），只返回username和mobile，会员中心页面需要显示更多字段，
    #     所以要创建一个userDetailSerializer
    # 2、问题又来了，如果注册的使用userdetailSerializer，又会导致严重失败，所以需要动态的使用serializer
    ...
    # 重写了get_object方法，就知道是哪个用户了【获取登录的用户】
    def get_object(self):
